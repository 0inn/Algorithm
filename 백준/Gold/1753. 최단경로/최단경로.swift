...
for _ in 0..<E {
    ...
    graph[node[0]].append(Node(n: node[1], w: node[2]))
}

var q = PriorityQueue<Node>(sort: <)

q.enqueue(Node(n: start, w: 0))
dist[start] = 0
...
while !q.isEmpty {
    ...
    for nxt in graph[cur.n] {
        if dist[nxt.n] > dist[cur.n] + nxt.w {
            dist[nxt.n] = dist[cur.n] + nxt.w
            q.enqueue(Node(n: nxt.n, w: dist[nxt.n]))
        }
    }
}

(1...V).forEach {
    dist[$0] == INF ? print("INF") : print(dist[$0])
}
